...
def build_common_args(args):
    cmd = ["--output", args.output, "--scale", str(args.scale)]
    if args.faction:
...
def main():
    ...
    parser.add_argument("--no-core-stratagems", action="store_true")
    ...
    if not args.no_core_stratagems:
        jobs.append([sys.executable, "export_core_stratagem_cards.py", "--output", args.output, "--scale", str(args.scale), *( ["--headed"] if args.headed else [] )])

    if not jobs:
        raise SystemExit("Nothing to export: every card type was disabled")
